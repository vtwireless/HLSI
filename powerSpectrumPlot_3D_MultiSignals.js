/**
 * The `PowerSpectrumPlot_3D` function generates a 3D Power Spectrum Plot with one or more signals based
 * on the provided options and signal parameters.
 *
 * @param {Object} [opts] - An options object for configuring the plot.
 * @param {boolean} [opts.has_signal=true] - A boolean flag indicating whether the plot should include signal data.
 * If `true`, the plot will display the signal; if `false`, it will omit the signal data.
 * @param {boolean} [opts.has_interferer=true] - A boolean flag indicating whether the plot should include an interferer signal.
 ...
 * based on the provided options, updating dynamically based on signal parameters. It also includes
 * interactive sliders for controlling time, x offset, and y offset.
 */
