/**
 * The JavaScript code contains helper functions to control the visibility of sliders based on mode
 * selection and to handle different modes for primary users in a communication system.
 */
/**
 * The above JavaScript code contains helper functions to control the visibility of sliders based on
 * mode selection and to handle different modes for primary users in a communication system.
 */
...
function onPUModeChange() {
  ...
  if (pu_mode === "interference" || pu_mode === "noninterferer") {
    ...
    if (signalList.length === 2) {
      SinrPlot_BarChart.init([sig], "sinr-bar-chart", ["User Signal"], null, [
        "rgb(48, 25, 72)",
        "rgba(230,97,0)",
      ]);
    } else {
      SinrPlot_BarChart.init([sig1, sig2, sig3, sig4], "sinr-bar-chart", [
        "Link 1",
        "Link 2",
        "Link 3",
        "Link 4",
