/**
 * The `PowerSpectrumPlot_3D` function generates a dynamic 3D power spectrum plot that includes signals
 * and noise, updating the plot as signal parameters change.
 *
 * @param {Object} [opts] - An options object to configure the plot.
 * @param {boolean} [opts.has_signal=true] - A flag indicating whether the plot should include a primary signal.
 * If `true`, the plot will display the signal; if `false`, it will exclude it.
 * @param {boolean} [opts.has_interferer=true] - A flag indicating whether the plot should include an interferer
 * component. If `true`, the plot will display the interferer; if `false`, it will exclude it.
 * @returns {void} This function does not return a value. It generates and displays a 3D Power Spectrum Plot
 * based on the provided options and updates dynamically with sliders for controlling time, x offset, and y offset.
 */
...
function PowerSpectrumPlot_3D(
  ...
  const svg = d3
    ...
    .property("value", { x: x });
  if (typeof opts.yMax === "undefined") opts.yMax = 20;
  if (typeof opts.yMin === "undefined") opts.yMin = -60;
  ...
  function update_plot() {
    ...
    let cur_interferer_freq = 0;
    let cur_signal_freq = 0;

    let cur_interferer_freq_exact = 0;
    let cur_signal_freq_exact = 0;

    let cur_interferer_bw = 0;
    let cur_signal_bw = 0;

    // let cur_interferer_psd = 0;
    // let cur_signal_psd = 0;
    ...
    sigs.forEach(function (sig) {
      ...
      if (sig["name"] == "interferer") {
        ...
        cur_interferer_bw = bw;
        cur_interferer_freq_exact = sig._freq;
      } else {
        cur_signal_freq = fc;
        cur_signal_bw = bw;
        cur_signal_freq_exact = sig._freq;
      }
    ...
    let interferer_bounding_box_new_coordinates = get_bounding_box_coordinates(
      750,
      320,
      cur_interferer_freq,
      cur_interferer_bw
    ...
    interferer_box_f.attr(
      "width",
      interferer_bounding_box_new_coordinates["width"]
    );
    interferer_box_f.attr(
      "height",
      interferer_bounding_box_new_coordinates["height"]
    );
    ...
    let interferer_label_new_coordinates = get_graph_label_coordinates(
      720,
      320,
      cur_interferer_freq
    );
    ...
    g
      ...
      .call(
        d3
          .axisBottom(x)
          .tickFormat((x) =>
            ((1780 * (nfft - x)) / nfft + (1820 * x) / nfft).toFixed(0)
          )
      );
  var yAxis = (g, y) =>
    g.attr("transform", `translate(${margin.left},0)`).call(d3.axisLeft(y));
  var polyline = (data, x, y) =>
  ...
  function draw() {
    ...
    svg
      ...
      .attr("points", function () {
        return changepos(d3.select(this).attr("points"));
      })
      .attr("style", function () {
        return (
          "opacity:" +
          (parseFloat(
            d3.select(this).attr("style").split(":")[1].split(";")[0]
          ) -
            1 / maxlinesgenerated) +
          "; stroke: steelblue;"
        );
