function bpskspat(X1Y1,X2Y2,XY1,XY2,XY3,XY4,XY5,XY6,active,freq,samp,Ts,bits,Eborange,pwr,gamma,los,flag1,flag2,berflag,flagap,flagap1,txd)
%%%
%%%
tic
mult1=[];
...
countnum=0;
for tt=1:length(active),
      totaltrans(tt,:)=timefilt(tt,:)*conj(c(uu));
...
%    hold on
   maxpat=antennapat(X2Y2,lambda,conj(c),mult1,anglemult1,mult2,anglemult2,mult3,anglemult3,mult4,anglemult4,mult5,anglemult5,mult6,anglemult6,usu,active)
...
%  end
  ...
  %pause
     ...
     noiseout(1,:)=conj(c(uu))*[sqrt(varnorm)*randn(1,samp)+j*sqrt(varnorm)*randn(1,samp)];
  ...
  if h>transient
     bitcount=bitcount+1;
     if (bitcount/50000)==floor(bitcount/50000);
       bitcount
       toc
       tic
     end
