...
function connectBoxes(draw, boxA, boxB) {
  const line = draw.line(0, 0, 0, 0).stroke({ width: 2 });
    console.log(boxA);
  ...
  function update() {
    ...
    line.plot(a.cx, a.cy, b.cx, b.cy).stroke({ color: '#fb0d0d', width: 2 });
    console.log(a.cx);
  }

  // store updater
  boxA.data('connections').push(update);
  boxB.data('connections').push(update);

  update();

  return line;
}
  function createBox(draw, x, y, labelText) {
  const group = draw.group();

  let rect = group.rect(120, 60)
    .move(x, y)
    .fill('#ef0d0d')
...
function createBlockDiagram(){



var draw = SVG().addTo('#svgContainer').size('100%', '100%')
  let box1 = createBox(draw, 50, 50, "Input");
  let box2 = createBox(draw, 250, 50, "Output");

  let line1 = connectBoxes(draw, box1, box2);
}
