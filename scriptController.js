/**
 * The `ScriptController` function creates a versatile text editor in JavaScript for the browser,
 * allowing users to write and execute custom JavaScript functions with specified parameters and options.
 *
 * @param {Object|Object[]} sigs - A signal object or an array of signal objects, each representing a
 * different signal with properties like frequency, bandwidth, gain, modulation coding scheme, and rate.
 * These signal objects are used for operations within the editor environment.
 * @param {Object|null} [opts=null] - An options object to customize the behavior and settings of the
 * `ScriptController`. This parameter is optional and can be left `null` to use default settings.
 * @returns {void} This function does not return a value. It sets up a user interface for editing and
 * running JavaScript code periodically, using the provided signals and options.
 */
...
function ScriptController(sigs, opts = null) {
  ...
  if (opts && !Array.isArray(opts.postfix)) opts.postfix = [];
  ...
  if (typeof opts.sync === "object" || Array.isArray(opts.sync)) {
    if (!Array.isArray(opts.sync)) opts.sync = [opts.sync];
  ...
  [10.0, 5.0, 4.0, 3.0, 2.0, 1.0, 0.75, 0.5, 0.25, 0.1, 0.05, 0.02].forEach(
    function (ddt) {
  ...
  if (obj._syncGroup) dtSelect.selectedIndex = obj._syncGroup.dtSelectedIndex;
  else dtSelect.selectedIndex = initDtSelectedIndex;
  ...
  minimizeBtn.onclick = function (e) {
    ...
    let editorCode = document.getElementsByClassName(
      "CodeMirror cm-s-blackboard"
    );
    minimizeFlag
      ? argsTable.setAttribute("style", "height:70px")
      : argsTable.setAttribute("style", "height:320px");
    minimizeFlag
      ? editorCode[editorId].setAttribute("style", "height:50px")
  ...
  if (opts && opts.customLabels) {
    if (sigs.length === 1 && p.id === "scriptController2") {
      funcDeclareSpan.appendChild(
        document.createTextNode("INTERFERER Controller (" + argsStr + "){")
      );
    } else {
      funcDeclareSpan.appendChild(
        document.createTextNode("SIGNAL Controller (" + argsStr + "){")
      );
    }
  } else {
    funcDeclareSpan.appendChild(
      document.createTextNode("Function Callback(" + argsStr + "){")
    );
  }
  ...
  editorDiv.appendChild(
    document.createTextNode("The above function will be called once every ")
  );
  ...
  tr.innerHTML =
    '<th scope="col" colspan="2" class=ph1>Callback Parameters</th>';
  thead.appendChild(tr);
  tr = document.createElement("tr");
  tr.innerHTML =
    '<th scope="col" class=ph2>Parameter</th>' +
  ...
  sigs.forEach(function (sig) {
    ...
    tr.innerHTML =
      "<td>freq" +
      fix +
      "</td class=p1><td class=p2>center frequency of signal " +
      fix +
      " in Hz</td>";
    ...
    tr.innerHTML =
      "<td class=p1>bw" +
      fix +
      "</td><td class=p2>bandwidth of signal " +
      fix +
      " in Hz</td>";
    tbody.appendChild(tr);
    ...
    tr.innerHTML =
      "<td class=p1>mcs" +
      fix +
      "</td><td class=p2>modulation coding scheme of signal " +
      fix +
      ", index 0 to 11</td>";
    tbody.appendChild(tr);
    ...
    tr.innerHTML =
      "<td class=p1>rate" +
      fix +
      "</td><td class=p2>rate of signal " +
      fix +
      ", read only, in bits/second</td>";
    tbody.appendChild(tr);
  ...
  tr = document.createElement("tr");
  tr.innerHTML =
    "<td class=p1>dt</td><td class=p2>time elapsed since last call, in seconds</td>";
  tbody.appendChild(tr);

  tr = document.createElement("tr");
  tr.innerHTML =
    "<td class=p1>userData</td><td class=p2>object preserved between calls</td>";
  tbody.appendChild(tr);

  tr = document.createElement("tr");
  tr.innerHTML =
    "<td class=p1>init</td><td class=p2>true when callback is first called, false otherwise</td>";
  tbody.appendChild(tr);

  tr = document.createElement("tr");
  tr.innerHTML =
    "<td class=p1>globalUserData</td><td class=p2>object preserved between calls and between all code</td>";
  tbody.appendChild(tr);
  ...
  function Start() {
    ...
    if (!obj._syncGroup)
      intervalId = setInterval(
        callUserFunction,
        Math.round(1000.0 * dt) /*milliseconds*/
      );
...
*/
  ...
  function makeWidget() {
    ...
    editor.on("change", function () {
      if (funcSelect.className !== "edited") funcSelect.className = "edited";
      if (running) Stop();
    });
  }
