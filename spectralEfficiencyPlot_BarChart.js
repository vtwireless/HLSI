/* This JavaScript code defines an object `SpectralEfficiencyPlot_BarChart` that handles the creation
and updating of a bar chart representing spectral efficiency data. Here is a breakdown of what the
code does: */
"use strict";
...
const SpectralEfficiencyPlot_BarChart = {
  ...
  update_plot: function () {
    ...
    let new_dataset = SpectralEfficiencyPlot_BarChart.generate_dataset(
      SpectralEfficiencyPlot_BarChart.signal_list
    );

    // console.log(new_dataset);

    let total_rate = SpectralEfficiencyPlot_BarChart.get_total_throughput(
      SpectralEfficiencyPlot_BarChart.signal_list
    );
    let total_bw = 0.0;
    let max_freq = 0.0;
    ...
    for (let i = 0; i < cur_dataset.length; i++) {
      ...
      if (new_value != cur_value) {
        SpectralEfficiencyPlot_BarChart.se_chart.data.datasets[0].data[i] =
          new_value;
      }
      ...
      let signal_freq_left_limit =
        signal_freq - SpectralEfficiencyPlot_BarChart.signal_list[i].bw / 2;
      let signal_freq_right_limit =
        signal_freq + SpectralEfficiencyPlot_BarChart.signal_list[i].bw / 2;

      if (signal_freq_right_limit > max_freq)
        max_freq = signal_freq_right_limit;
      if (signal_freq_left_limit < min_freq) min_freq = signal_freq_left_limit;
    }
    ...
    SpectralEfficiencyPlot_BarChart.se_chart.data.datasets[0].data[4] =
      average_se;
