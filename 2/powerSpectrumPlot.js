...
function PowerSpectrumPlot(sigs, parentElement=null) {
    ...
    const m = 40, k=20, n = 2*k*m+1;
    var bw = 0.2, gn = 0.0;
    var nfft = 2048, generator = new siggen(nfft);
    generator.m = m;

    // determine time and frequency scale/units
    var [scale_freq,units_freq] =
        scale_units(sigs[0].freq_init+sigs[0].bw_max*0.5,0.1); // freq scale
    ...
    var linef = d3.line()
        .x(function(d, i) {
            return fScale( (freq_plot_min + i*df)*scale_freq); })  // map frequency
    ...
    // clip paths
    svgf.append("clipPath").attr("id","clipf")
        .append("rect")
    ...
    function update_plot() {
        ...
        sigs.forEach(function(sig) {
            var fc = - 0.5 + (sig.freq - sig.freq_plot_min)/(
