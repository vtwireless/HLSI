/**
 * The `ThroughputPlot` function creates a dynamic plot showing the integral of a signal's rate over time,
 * compared to a specified threshold. It can handle multiple signals and interferers.
 *
 * @param {Array<Object>} signals - An array of signal objects for which the throughput plot is created.
 * Each signal object should contain information such as the signal's name and rate. The plot will
 * display the historical rate data of these signals over time.
 * @param {Array<Object>} [interferers] - An optional array of interferer signal objects that impact
 * the main signals' throughput. If not provided, the function defaults to using the signals in `Signal.env`.
 * @param {number|null} [threshold=null] - An optional threshold value for the data rate in bits per second.
 * If provided, a horizontal line will be added to the plot at the threshold, indicating when the rate
 * exceeds or falls below this value.
 * @returns {void} This function does not return a value. It creates and displays a throughput plot
 * using D3.js, visualizing the integral of signal rates over time relative to the optional threshold.
 */
...
var total_data_rate_signal = Signal(
  JSON.parse(JSON.stringify(conf.signal_multi)),
  "",
  {
    name: "total_rate",
  }
...
total_data_rate_signal.onChange = function (par, callback) {
  if (typeof total_data_rate_signal._callbacks[par] !== "undefined") {
    ...
    console.log(
      "ERROR: you can't add a '" +
        par +
        "' callback to signal " +
        total_data_rate_signal.name
    );
};
...
function ThroughputPlot(signals, interferers = [], threshold = null) {
  ...
  var tscale = d3
    .scaleLinear()
    .domain([-plot_period * num_steps, 0])
    .range([0, width]);
  ...
  var rtickvalues = [
    100e3, 400e3, 1e6, 2e6, 4e6, 10e6, 20e6, 40e6, 100e6, 200e6, 400e6,
  ];
  svgr
    .append("g")
    .attr("class", "y axis")
  ...
  svgr
    .append("g")
    .attr("class", "grid")
    .call(d3.axisBottom(tscale).tickFormat("").tickSize(height));
  ...
  svgr
    ...
    .attr(
      "transform",
      "translate(" + width / 2 + "," + (height + 0.75 * margin.bottom) + ")"
    )
    .attr("dy", "-0.3em")
  ...
  for (let i = 0; i < signal_list.length; i++) {
    signal_list[i].datar = d3.range(0, num_steps).map(function (f) {
      return { y: 1 };
    });
    signal_list[i].liner = d3
      .line()
      .x(function (d, i) {
        return tscale(-plot_period * (num_steps - i));
      })
      .y(function (d, i) {
        return rscale(d.y);
      });
    // 9. Append the path, bind the data, and call the line generator
    signal_list[i].pathr = svgr
      .append("path")
  ...
  total_data_rate_signal.datar = d3.range(0, num_steps).map(function (f) {
    return { y: 1 };
  });
  total_data_rate_signal.liner = d3
    .line()
    .x(function (d, i) {
      return tscale(-plot_period * (num_steps - i));
    })
  ...
  total_data_rate_signal.pathr = svgr
    .append("path")
    .attr("clip-path", `url(#clipr)`)
    ...
    .attr(
      "style",
      `stroke:${signal_colors_stroke[signal_colors_stroke.length - 1]};`
    )
  ...
  function update_plot() {
    if (ThroughputPlot.timerMode && ThroughputPlot.timeLeft > 0) {
      ...
      if (ThroughputPlot.timeLeft < 0.0) ThroughputPlot.timeLeft = 0.0;

      if (
        ThroughputPlot.timeLeft === 0.0 &&
        ThroughputPlot.timerMode &&
